...
import vdp_sys as sys

plt.close('all')

# initialize parameters
nx = 2  # dimension of states
ny = 2  # dimension of outputs
...
kp_obj = koopman.KoopmanModel(nx, ny, nu, n_rbf, n_sim, n_traj, rbf_type, n0 = 100, nd = nd)
# define motor object
...
for i in range(n_sim2):
    ...
    # u = np.sign(np.sin(i/10))  # random input
    u = 0
...
plt.plot(itera_num * t_step, x_true[0, :], label='True y')
...
plt.plot(x_true[0, :], x_true[1, :], label='True y')
plt.plot(x_koop[0, :], x_koop[1, :], label='Koopman y')
plt.xlabel('x1')
plt.ylabel('x2')
plt.legend()
